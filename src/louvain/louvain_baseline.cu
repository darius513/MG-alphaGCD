...
nvshmem_double_atomic_add(
        double* address,
        double val,
        int dst_pe
)
{
    long long* addr_as_ll = (long long*)address;
    long long old = nvshmem_longlong_g(addr_as_ll, dst_pe);
    long long assumed;
    if (val == 0.0)
    ...
    {
        assumed = old;
        old = nvshmem_longlong_atomic_compare_swap(addr_as_ll, assumed, __double_as_longlong(__longlong_as_double(assumed) + val), dst_pe);
    } while(assumed != old);
...
)
{
    ...
    for (vertex_id = grid.thread_rank(); vertex_id < local_vertices; vertex_id += grid.num_threads()) {
        ...
        if (src_community_id != dst_community_id) {
            ...
            // Updates the community weight that the vertex originally belonged to
            locating_vertex(pe_dst, src_community_id, private_device_part_vertex_offset, n_pes);
            if (pe_dst == my_pe) {
                atomicAdd(shared_device_community_weight + src_community_id, -vertex_weight);
            } else {
                nvshmem_double_atomic_add(shared_device_community_weight + src_community_id, -vertex_weight, pe_dst);
            }

            // Update the target community weights for vertex
            locating_vertex(pe_dst, dst_community_id, private_device_part_vertex_offset, n_pes);
            if (pe_dst == my_pe) {
                atomicAdd(shared_device_community_weight + dst_community_id, vertex_weight);
            } else {
                nvshmem_double_atomic_add(shared_device_community_weight + dst_community_id, vertex_weight, pe_dst);
            }
        }
    }
...
void louvain_baseline::run(HostGraph *hostGraph, GpuGraph *gpuGraph, const double threshold, const int max_iter,
